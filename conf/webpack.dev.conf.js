      ...
      rewrites: [
        {
          from: /.*/,
          to: function (opt) {
            const userAgent = opt.request.headers['user-agent']
            let indexFile = 'app.html'
            if (regex1.test(userAgent)) {
              indexFile = 'mobile.html'
            } else if (regex2.test(userAgent)) {
              indexFile = 'mobile.html'
            }
            return path.posix.join('/', indexFile)
          }
        }
      ]
    }
